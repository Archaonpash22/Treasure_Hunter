...
class MapBridge(QObject):
    ...
    Signals are used to send data from JavaScript to Python.
    """
    log_requested = pyqtSignal(str) # Signal for logging messages from JS
    map_ready = pyqtSignal() # Signal emitted when the Leaflet map is fully initialized
    map_right_clicked = pyqtSignal(float, float) # Signal for map right-click coordinates

    @pyqtSlot(str)
    def log(self, message):
        """Receives log messages from JavaScript and emits them to Python."""
        self.log_requested.emit(message)

    @pyqtSlot()
    def onMapReady(self):
        """Receives map ready signal from JavaScript and emits it to Python."""
        self.map_ready.emit()

    @pyqtSlot(float, float)
    def onMapRightClicked(self, lat, lon):
        """Receives right-click coordinates from JavaScript and emits them to Python."""
        self.map_right_clicked.emit(lat, lon)

class MapWidget(QWidget):
    """
    A PyQt widget that embeds a web engine view to display an interactive Leaflet map.
    Handles loading the map, running JavaScript commands, and managing map layers/markers.
    """
    def __init__(self, parent=None):
        super().__init__(parent)
        self.bridge = MapBridge() # Instantiate the bridge for JS-Python communication
        self.init_ui() # Initialize the UI components
        self.create_map_html() # Generate the HTML content for the map
        self.load_map() # Load the HTML map into the web view

    def init_ui(self):
        """
        Initializes the user interface for the MapWidget, primarily the QWebEngineView.
        """
        layout = QVBoxLayout(self)
        layout.setContentsMargins(0,0,0,0) # Remove margins around the web view

        self.web_view = QWebEngineView()
        # Allow local HTML content to access remote URLs (e.g., Leaflet CDN)
        self.web_view.settings().setAttribute(QWebEngineSettings.WebAttribute.LocalContentCanAccessRemoteUrls, True)

        self.channel = QWebChannel(self.web_view.page())
        self.web_view.page().setWebChannel(self.channel)
        # Register the MapBridge object so JavaScript can access its slots
    ...
    def create_map_html(self):
        """
        Generates the HTML content for the Leaflet map and writes it to map.html.
        This HTML includes Leaflet library, custom JavaScript for map interactions,
        ...
        """
        # Write the generated HTML content to the map.html file
        with open(HTML_MAP_FILE, "w", encoding="utf-8") as f:
            f.write(html_content)

    def load_map(self):
        """
        Loads the generated map.html file into the QWebEngineView.
        Clears the HTTP cache to ensure the latest HTML is loaded.
        """
        profile = QWebEngineProfile.defaultProfile()
        profile.clearHttpCache() # Clear cache to avoid stale content
        self.web_view.setUrl(QUrl.fromLocalFile(os.path.abspath(HTML_MAP_FILE)))

    def run_js(self, script):
        """
        Executes a JavaScript script within the QWebEngineView.
        """
        self.web_view.page().runJavaScript(script)

    def center_on_location(self, lat, lon, popup_text):
        """
        Centers the map on a given latitude and longitude and displays a popup.
        Text is escaped for JavaScript compatibility.
        """
    ...
    def add_permanent_marker(self, marker_data):
        """
        Adds a permanent marker to the map using data from Python.
        """
    ...
    def update_all_markers(self, markers_list):
        """
        Updates all permanent markers on the map by clearing existing ones
        and adding new ones from the provided list.
        """
        self.run_js(f"window.updateAllMarkers({json.dumps(markers_list)});")

    def toggle_poi_layer(self, layer_key, is_visible, data):
        """
        Toggles the visibility of a specific POI layer on the map.
        The data is passed to JavaScript for marker creation.
        """
        # Diese Methode ruft jetzt die neue JS-Funktion auf
