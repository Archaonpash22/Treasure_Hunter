...
class MapBridge(QObject):
    ...
    # New signal to request a URL fetch from Python
    fetch_url_requested = pyqtSignal(str, str) # request_id, url
...
class MapWidget(QWidget):
    ...
    def init_ui(self):
        ...
        # This setting is necessary to allow requests to external servers (like the DARE map)
    ...
    def create_map_html_file(self):
        """
        Creates the map.html file with all necessary JavaScript, including the CORS proxy logic.
        """
        # This is the minified and corrected version of leaflet.utfgrid.js
        utfgrid_script_content = """
        /*
         (c) 2012, Michael Evans, Thomas J Bradley, GEOIQ, David vt., KlokanTech.com
        */
        ...
        <script>
            ...
            function initializeMap() {{
                // ### FIX: Override Leaflet's AJAX function to use our Python bridge ###
                ...
                for (const countryName in terrainData) {{
                    ...
                    for (const layerKey in country.layers) {{
                        if (!terrainLayers[layerKey]) {{
                            ...
                            if (crs_string && typeof crs_string === 'string' && crs_string === 'EPSG:3857') {{
        ...
        return """
            ...
            window.togglePoiLayerVisibility = function(layerKey, show) {
                ...
                if (show) {
                    ...
                    data.forEach(poi => {
                        // ### FIX: Added check to prevent error on invalid POI data ###
            ...
            window.addPermanentMarker = function(data) {
                // ### FIX: Added check to prevent error on invalid marker data ###
    ...
    def load_map(self):
        """
        Loads the map from a local file, which is more reliable than a local server.
        """
